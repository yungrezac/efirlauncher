...
function startWalletGateway({url,key,getToken}) {
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  try {
   ...
   if(req.method!=='POST'||(!leaderboard&&req.url!=='/wallet')||!sessions.has(req.headers.authorization)||req.headers.origin)throw Error('Нет доступа');
   ...
   if(!(leaderboard?['list','report']:['list','credit','reserve','commit','cancel','profile']).includes(args.p_action))throw Error('Неизвестная операция');
   const response=await fetch(url+'/rest/v1/rpc/'+(leaderboard?'flappy_leaderboard':'timer_wallet'),{method:'POST',headers:{apikey:key,Authorization:'Bearer '+accessToken,'Content-Type':'application/json'},body:JSON.stringify(args),signal:AbortSignal.timeout(15000)});
